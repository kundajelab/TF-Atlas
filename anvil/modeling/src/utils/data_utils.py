...
def get_cords(peaks_df, genome, width):
    """
    Same as get_cts, but fetches sequence from a given genome.
    """
    vals = []
    for i, r in peaks_df.iterrows():
        vals.append([r['chr'], r['start']+r['summit'] - width//2, r['start'] + r['summit'] + width//2])
...
def load_test_data(peaks_file, nonpeaks_file, genome_fasta, cts_bw_file,
    ...
    peak_coords = get_cords(test_peaks_df, genome, inputlen)
    ...
    test_nonpeaks_seqs, test_nonpeaks_cts = get_seq_cts(test_nonpeaks_df,
                                                        ...
                                                        inputlen,
                                                        outputlen)
    non_peak_coords = get_cords(test_nonpeaks_df, genome, inputlen)


    cts_bw.close()
    genome.close()

    return test_peaks_seqs, test_peaks_cts, test_nonpeaks_seqs, test_nonpeaks_cts, peak_coords, non_peak_coords
