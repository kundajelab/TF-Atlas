...
if [ "$assay_type" = "DNase-seq" ] ; then
    ...
    echo $( timestamp ): "samtools view -b" $input_bam "|" "bedtools bamtobed -i stdin" "|" \
    "awk -v OFS=\"\t\" \'{if ($6==\"-\"){print \$1,\$2,\$3+1,\$4,\$5,\$6} else if (\$6==\"+\") {print \$1,\$2,\$3,\$4,\$5,\$6}}\' |" \
    "bedtools genomecov -bg -5 -i stdin -g" $chrom_sizes "| bedtools sort -i stdin >" $intermediates_dir/$experiment$tag.bedGraph | \
    tee -a $logfile

    samtools view -b $input_bam | bedtools bamtobed -i stdin | \
    awk -v OFS="\t" '{if ($6=="-"){print $1,$2,$3+1,$4,$5,$6} else if ($6=="+") {print $1,$2,$3,$4,$5,$6}}' | \
    bedtools genomecov -bg -5 -i stdin -g $chrom_sizes | bedtools sort -i stdin > $intermediates_dir/$experiment$tag.bedGraph

else [ "$assay_type" = "ATAC-seq" ] ; then
